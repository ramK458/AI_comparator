...
# ---------------------------------------------------------------------------
# compute_minutes (v4 — daily compute derived from cost-sheet timestamps)
# ---------------------------------------------------------------------------

def test_cost_sheet_captures_compute_minutes(tmp_path):
    # Two models sharing the SAME day-bucket span on one day -> dedup to 1440
    # min (NOT 2880 from the flash+pro rows); a day with tokens but NO cost
    # rows -> 0.0.
    zp = _make_zip(
        tmp_path, "usage_data_2026-06-01_2026-06-30.zip",
        cost_rows=[
        ...
        amount_rows=[
            "u,2026-06-03T00:00:00+02:00,2026-06-04T00:00:00+02:00,deepseek-v4-flash,k,n,"
            "input_cache_hit_tokens,0.0000000028,1000000",
        ],
    )
    usage = uc.load_usage([zp])
    ...
    assert set(by_day) == {"2026-06-01", "2026-06-02", "2026-06-03"}
    # same 24h span shared by flash+pro -> deduped to 1440, not 2880
    assert by_day["2026-06-01"]["compute_minutes"] == pytest.approx(1440.0)
    # distinct day-bucket span on day 2
    assert by_day["2026-06-02"]["compute_minutes"] == pytest.approx(1440.0)
    # day with tokens but no cost rows -> 0.0
    assert by_day["2026-06-03"]["compute_minutes"] == pytest.approx(0.0)
    # internal span-dedup set is not part of the public day shape
    assert "_spans" not in usage["months"]["2026-06"]["days"]["2026-06-01"]


def test_compute_minutes_sums_distinct_session_spans(tmp_path):
    # TWO distinct short spans on the same day -> 30 + 15 = 45 min.
    zp = _make_zip(
        tmp_path, "usage_data_2026-06-01_2026-06-30.zip",
        cost_rows=[
            "u,2026-06-01T09:00:00+02:00,2026-06-01T09:30:00+02:00,deepseek-v4-flash,Paid,1.0,USD",
            "u,2026-06-01T14:00:00+02:00,2026-06-01T14:15:00+02:00,deepseek-v4-pro,Paid,2.0,USD",
        ],
    )
    usage = uc.load_usage([zp])
    rows = uc.daily_summary(usage)
    ...
    assert rows[0]["compute_minutes"] == pytest.approx(45.0)


def test_daily_summary_includes_compute_minutes():
    usage = _usage_from_days({
        "2026-06": {
            "2026-06-01": {"flash": {"hit": 100.0, "miss": 0.0, "out": 0.0}},
            "2026-06-02": {"flash": {"hit": 0.0, "miss": 0.0, "out": 0.0}},
        },
    })
    # inject compute_minutes on one day only
    usage["months"]["2026-06"]["days"]["2026-06-02"]["compute_minutes"] = 30.0
    rows = uc.daily_summary(usage)
    by_day = {r["day"]: r for r in rows}
    # day WITHOUT compute_minutes -> 0.0
    assert by_day["2026-06-01"]["compute_minutes"] == 0.0
    # day WITH compute_minutes -> 30.0
    assert by_day["2026-06-02"]["compute_minutes"] == pytest.approx(30.0)


def test_span_minutes_parse_and_errors():
    # valid short span -> 30 min
    assert uc._span_minutes("2026-06-01T09:00:00+02:00",
                            "2026-06-01T09:30:00+02:00") == pytest.approx(30.0)
    # day-bucket span -> 24h = 1440 min
    assert uc._span_minutes("2026-06-01T00:00:00+02:00",
                            "2026-06-02T00:00:00+02:00") == pytest.approx(1440.0)
    # invalid / empty / None timestamps -> 0.0
    assert uc._span_minutes("not-a-date", "2026-06-01T09:30:00+02:00") == 0.0
    assert uc._span_minutes("", None) == 0.0
    assert uc._span_minutes(None, None) == 0.0
    # end before start -> 0.0 (never negative)
    assert uc._span_minutes("2026-06-01T09:30:00+02:00",
                            "2026-06-01T09:00:00+02:00") == 0.0
